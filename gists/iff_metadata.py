...
def main():
    ...
    for fname in os.listdir(directory):
        ...
        num_meta = fd.get('SI.hStackManager.framesPerSlice') or fd.get('SI.hRoiManager.framesPerSlice')
        series_frames = tf.series[0].shape[0]
        pages = len(tf.pages)
        print(f"{fname}: meta_frames={num_meta}, series_frames={series_frames}, pages={pages}")
