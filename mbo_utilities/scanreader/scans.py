...
class BaseScan:
    ...
    def version(self):
        match = re.search(
            r"SI.?\.VERSION_MAJOR = '?(?P<version>[^\s']*)'?", self.header
        )
    ...
    def num_scanning_depths(self):
        if self.is_slow_stack:
            ...
            num_scanning_depths = self._num_pages / (
                self.num_channels * self.num_frames
            )
            num_scanning_depths = int(
                num_scanning_depths
            )  # discard last slice if incomplete
        else:
    ...
    def num_requested_frames(self):
        if self.is_slow_stack:
            match = re.search(
                r"hStackManager\.framesPerSlice = (?P<num_frames>.*)", self.header
            )
        else:
        ...
        num_requested_frames = (
            int(
                1e9
                if match.group("num_frames") == "Inf"
                else float(match.group("num_frames"))
            )
            if match
            else None
        )
    ...
    def num_frames(self):
        ...
        if self.is_slow_stack:
            num_frames = min(
                self.num_requested_frames / self._num_averaged_frames,
                self._num_pages / self.num_channels,
            )  # finished in the first slice
        else:
            num_frames = self._num_pages / (
                self.num_channels * self.num_scanning_depths
            )
    ...
    def is_bidirectional(self):
        match = re.search(
            r"hScan2D\.bidirectional = (?P<is_bidirectional>.*)", self.header
        )
        is_bidirectional = (
            (match.group("is_bidirectional") == "true") if match else False
        )
    ...
    def scanner_frequency(self):
        match = re.search(
            r"hScan2D\.scannerFrequency = (?P<scanner_freq>.*)", self.header
        )
    ...
    def seconds_per_line(self):
        if np.isnan(self.scanner_frequency):
            match = re.search(
                r"hRoiManager\.linePeriod = (?P<secs_per_line>.*)", self.header
            )
            seconds_per_line = float(match.group("secs_per_line")) if match else None
        else:
            scanner_period = (
                1 / self.scanner_frequency
            )  # secs for mirror to return to initial position
            seconds_per_line = (
                scanner_period / 2 if self.is_bidirectional else scanner_period
            )
    ...
    def _num_averaged_frames(self):
        ...
        match = re.search(
            r"hScan2D\.logAverageFactor = (?P<num_avg_frames>.*)", self.header
        )
        num_averaged_frames = (
            int(float(match.group("num_avg_frames"))) if match else None
        )
    ...
    def spatial_fill_fraction(self):
        match = re.search(
            r"hScan2D\.fillFractionSpatial = (?P<spatial_ff>.*)", self.header
        )
    ...
    def temporal_fill_fraction(self):
        match = re.search(
            r"hScan2D\.fillFractionTemporal = (?P<temporal_ff>.*)", self.header
        )
    ...
    def motor_position_at_zero(self):
        ...
        match = re.search(
            r"hMotors\.motorPosition = (?P<motor_position>.*)", self.header
        )
        motor_position = (
            matlabstr2py(match.group("motor_position"))[:3] if match else None
        )
    ...
    def _num_fly_back_lines(self):
        ...
        match = re.search(
            r"hScan2D\.flybackTimePerFrame = (?P<fly_back_seconds>.*)", self.header
        )
        if match:
    ...
    def _compute_offsets(self, field_height, start_line):
        ...
        if self.is_bidirectional:  # odd lines scanned from left to right
            field_offsets[1::2] = (
                field_offsets[1::2] - line_offsets + (1 - line_offsets)
            )
...
class BaseScan5(BaseScan):
    ...
    def is_slow_stack_with_fastZ(self):
        match = re.search(
            r"hMotors\.motorSecondMotorZEnable = (?P<uses_fastZ>.*)", self.header
        )
    ...
    def _y_angle_scale_factor(self):
        ...
        match = re.search(
            r"hRoiManager\.scanAngleMultiplierSlow = (?P<angle_scaler>.*)", self.header
        )
    ...
    def _x_angle_scale_factor(self):
        ...
        match = re.search(
            r"hRoiManager\.scanAngleMultiplierFast = (?P<angle_scaler>.*)", self.header
        )
    ...
    def __getitem__(self, key):
        ...
        if isinstance(full_key[1], list) and isinstance(full_key[2], list):
            ...
            item = item.reshape(
                ...
                len(channel_list),
                len(frame_list),
            )  # put back any dropped dimension
...
class Scan5Point2(BaseScan5):
    ...
    def image_height_in_microns(self):
        match = re.search(
            r"hRoiManager\.imagingFovUm = (?P<fov_corners>.*)", self.header
        )
        if match:
...
class NewerScan:
    ...
    def is_slow_stack_with_fastZ(self):
        match = re.search(
            r"hStackManager\.slowStackWithFastZ = (?P<slow_with_fastZ>.*)", self.header
        )
        slow_with_fastZ = (
            (match.group("slow_with_fastZ") in ["true", "1"]) if match else None
        )
        return slow_with_fastZ


class Scan5Point3(
    NewerScan, Scan5Point2
...
class ScanMultiROI(NewerScan, BaseScan):
    ...
    def _num_fly_to_lines(self):
        ...
        match = re.search(
            r"hScan2D\.flytoTimePerScanfield = (?P<fly_to_seconds>.*)", self.header
        )
        if match:
