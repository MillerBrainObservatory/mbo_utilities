...
def stat_to_masks(stat, Ly, Lx, label_map=None):
    ...
    Attempts to use lbm_suite2p_python.conversion if available.
    ...
    try:
        from lbm_suite2p_python import conversion
        if hasattr(conversion, "stat_to_masks"):
             # Signature check or just try
             try:
                 return conversion.stat_to_masks(stat, Ly, Lx)
             except Exception:
                 pass
    except ImportError:
        pass
    ...
    for k, s in enumerate(stat):
        id_ = k + 1 
        if label_map:
            # If label_map provided, use it to map index k to ID
            # If k not in map, skip
...
def masks_to_stat(masks: np.ndarray) -> list[dict]:
    """
    ...
    -------
    list[dict]
        List of dicts with 'ypix', 'xpix', 'lam', 'med', 'npix'.
    """
    if masks.ndim != 2:
    ...
    for label in unique_labels:
        ...
        # Calculate specialized fields
        center_y = np.mean(ypix)
        center_x = np.mean(xpix)
        
        # lam (weights) - uniform for converted masks usually
...
class SegmentationMixin:
    ...
    def detect_cellpose(
        ...
        **kwargs
        ...
        try:
            ...
            raise ImportError("cellpose not installed. Install with `uv pip install cellpose`")
    ...
    def detect_suite2p(
        ...
        logger.warning("Suite2p detection from array not fully implemented. Use full run_s2p pipeline usually.")
        return []
    ...
    def save_segmentation(
        ...
        # Metadata
        labels_grp.attrs["labels"] = list(set(labels_grp.attrs.get("labels", []) + [name]))
    ...
    def export_to_cellpose(
        ...
        if masks is not None:
            ...
            tifffile.imwrite(mask_path, masks.astype(np.uint16)) # Cellpose likes uint16 usually
