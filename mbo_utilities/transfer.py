...
def validate_paths(data_path, remote_path):
    ...
    if not data_path.exists() or not data_path.is_dir():
        print(f"❌ Error: '{data_path}' must exist and be a directory.", file=sys.stderr)
        sys.exit(1)
    if " " in remote_path:
        print(f"❌ Error: Remote path '{remote_path}' contains spaces.", file=sys.stderr)
        sys.exit(1)
...
def transfer_file(host, user, local_file, remote_file_path, pbar, pbar_lock):
    try:
        ...
        if transferred < file_size:
            raise ValueError(f"Incomplete transfer for {local_file}: {transferred}/{file_size} bytes.")
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for f in all_files:
                ...
                futures.append(executor.submit(
                    transfer_file,
                    host, user, f, remote_file_path, pbar, pbar_lock
                ))
...
def main():
    ...
    if not args.remote_path:
        ...
        remote_path = f"/lustre/fs4/mbo/scratch/{args.user}/{args.remote_path.strip('/')}"

    local_dir, remote_path = validate_paths(args.data, remote_path)
    transfer_files(args.remote_host, args.user, local_dir, remote_path, exclude_patterns=args.exclude)
